...
export const useDashboardData = (selectedPotline, activeTab = 'Last CE') => {
    ...
    const [summaryStats, setSummaryStats] = useState({ total: 0, optimal: 0, warning: 0, critical: 0, average: 0 });
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const potlineId = selectedPotline === 'ALL POTLINE'
                    ? null
                ...
                const processedPots = rows.map(row => {
                    ...
                    let bt = row.bt || row.bath_temperature || row.bath_temp || 0;
                    let v = row.avv || row.voltage || row.volt || 0;
                    let noise = row.noise || 0;
                    let m = row.m || row.metal_level || 0;
                    let aef = row.aef || row.anode_effect || 0;
                    let age = row.age_day || 0;
                    ...
                    else if (finalCe === null) status = 'offline'; // No prediction available
                    else if (finalCe === 0 && !isPrediction) status = 'offline'; // Only offline if actual CE is 0
                    else if (finalCe === 0) status = 'offline';
                    else if (finalCe > 90) status = 'optimal'; // Updated to match new standard (> 90)
                ...
                const optimal = processedPots.filter(p => p.status === 'optimal').length;
                const warning = processedPots.filter(p => p.status === 'warning').length;
                const critical = processedPots.filter(p => p.status === 'critical').length;
                const offline = processedPots.filter(p => p.status === 'offline').length;

                // Average only includes active pots (non-zero)
                const activePotsCount = optimal + warning + critical;
                const avg = activePotsCount > 0
                    ? processedPots.filter(p => p.value > 0).reduce((acc, p) => acc + p.value, 0) / activePotsCount
                    : 0;
                ...
                const calculateCategoryAvg = (status) => {
                    ...
                    return categoryPots.reduce((acc, p) => acc + p.value, 0) / categoryPots.length;
                };
                ...
                if (activeTab === 'Predicted CE') {
                    ...
                    const avgActual = countActualActive > 0 ? sumActualCE / countActualActive : 0;
                ...
                setSummaryStats({
                    total, optimal, warning, critical, offline, average: avg,
                    trends,
                    avgOptimal, avgWarning, avgCritical
                });
    ...
    return { pots, summaryStats, donutData, highCEData, downCEData, loading, error };
};
