...
import { BarChart2, CheckCircle, AlertTriangle, XCircle, Activity } from 'lucide-react';

const Card = ({ icon: Icon, title, value, subtext, colorClass, iconColor, trend, isInverse = false, type, total = 0, extraData = {}, categoryAvg = 0 }) => {
        ...
        if (['OPTIMAL', 'WARNING', 'CRITICAL'].includes(type) && categoryAvg > 0) {
            ...
            return (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                    <span style={{ fontSize: '0.6rem', color: '#94a3b8', fontWeight: '600', textTransform: 'uppercase', marginBottom: '1px' }}>Avg CE</span>
                    <span style={{ fontSize: '1.1rem', fontWeight: '700', color: iconColor, lineHeight: '1' }}>
                        {categoryAvg.toFixed(2)}<span style={{ fontSize: '0.65rem' }}>%</span>
                    </span>
                </div>
            );
        } else if (type === 'TOTAL') {
    ...
    return (
        <div className="summary-card">
            <div className="summary-header" style={{ justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
                ...
                {trend !== undefined && trend !== 0 && (
                    <div className={`trend-indicator ${trendClass}`}>
                        {trendSymbol}{trend}{title.includes('AVERAGE') ? '%' : ''}
                    </div>
                )}
            </div>
            ...
            <div className="card-content-wrapper">
                <div className="card-left-content">
                    <div className="summary-value" style={{ color: ['#22c55e', '#f59e0b', '#ef4444'].includes(iconColor) ? iconColor : '' }}>
                        {value}
                    </div>
                    <div className="summary-subtext">{subtext}</div>
                </div>

                <div className="card-visual-right" style={{ width: 'auto', minWidth: 'auto' }}>
                    {renderVisual()}
                </div>
            </div>
        </div>
...
const SummaryCards = ({ data }) => {
    ...
    const {
        total = 0, optimal = 0, warning = 0, critical = 0, average = 0,
        trends = {}, offline = 0,
        avgOptimal = 0, avgWarning = 0, avgCritical = 0
    } = data || {};
