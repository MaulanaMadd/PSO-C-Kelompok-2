...
import { Search, Bell, Settings, ChevronDown, Moon, Sun, Check, X, Trash2, Menu, LayoutGrid } from 'lucide-react';
...
const Header = ({
    ...
    const formatDate = (date) => {
        ...
        return displayDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' });
    };
    ...
    return (
        <>
            ...
            <div className={`dashboard-header ${isScrolled ? 'scrolled' : ''}`}>
                <Link to="/dashboard" className="header-left" style={{ textDecoration: 'none', color: 'inherit' }}>
                ...
                <div className="header-right">
                    <div className={`theme-toggle-switch ${isDarkMode ? 'dark' : 'light'}`} onClick={toggleTheme} title="Toggle Theme">
                        <div className="theme-toggle-thumb">
                            {isDarkMode ?
                                <Moon size={16} className="thumb-icon" fill="white" color="white" /> :
                                <Sun size={16} className="thumb-icon" fill="white" color="white" />
                            }
                        </div>
                    ...
                    <button
                        className="icon-btn"
                        title={location.pathname === '/potline-map' ? "Dashboard View" : "Grid View"}
                        onClick={() => location.pathname === '/potline-map' ? navigate('/dashboard') : navigate('/potline-map')}
                        style={{
                            background: location.pathname === '/potline-map' ? (isDarkMode ? 'rgba(59, 130, 246, 0.15)' : '#eff6ff') : undefined,
                            color: location.pathname === '/potline-map' ? (isDarkMode ? '#60a5fa' : '#2563eb') : undefined,
                            borderColor: location.pathname === '/potline-map' ? (isDarkMode ? 'rgba(59, 130, 246, 0.3)' : '#bfdbfe') : undefined,
                            width: '40px',
                            height: '40px',
                            padding: 0,
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'center',
                            borderStyle: location.pathname === '/potline-map' ? 'solid' : 'none',
                    ...
                    <div
                        ...
                        title="View Profile"
                    >
                        <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user?.full_name || 'User')}&background=3b82f6&color=fff`} alt="User" />
                    </div>
                    <button
            ...
            {showControls && (
                <div className={`controls-bar ${isMobileMenuOpen ? 'mobile-open' : ''}`}>
