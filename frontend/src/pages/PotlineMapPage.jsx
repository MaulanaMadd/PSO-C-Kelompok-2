...
const PotlineMapPage = ({ isDarkMode, toggleTheme }) => {
    ...
    const { pots, summaryStats, loading, error } = useDashboardData(selectedPotline);
    ...
    const filteredDisplayPots = isSearching
        ? fullPotList.filter(pot => pot.name.toLowerCase().includes(searchQuery.toLowerCase()) || pot.id.toString().includes(searchQuery))
        : fullPotList;
    ...
    const generateBlocks = () => {
        ...
        const baseBlocks = [
            // Row 1 Left: Block 1-2 & 2-2
            {
                ...
                suffix: '2',
                count: 42,
                series1Start: 122, series1End: 142,
                series2Start: 222, series2End: 242
            },
            // Row 1 Right: Block 1-1 & 2-1 (Starts here per user request)
            {
                ...
                suffix: '1',
                count: 42,
                series1Start: 101, series1End: 121,
                series2Start: 201, series2End: 221
            },
            // Row 2 Left: Block 1-4 & 2-4
            {
                ...
                suffix: '4',
                count: 44,
                series1Start: 164, series1End: 185,
                series2Start: 264, series2End: 285
            },
            // Row 2 Right: Block 1-3 & 2-3
            {
                ...
                suffix: '3',
                count: 42,
                series1Start: 143, series1End: 163,
                series2Start: 243, series2End: 263
            }
        ];
    ...
    const getBlockDisplayItems = (block, allPots) => {
        ...
        const pots1 = allPots.filter(p => p.id >= block.series1Start && p.id <= block.series1End);
        const pots2 = allPots.filter(p => p.id >= block.series2Start && p.id <= block.series2End);
    ...
    const getStatusColor = (type, value) => {
        ...
        switch (type) {
            case 'bt':
                ...
                if ((val >= 925 && val < 940) || (val > 960 && val <= 975)) return 'status-warning';
            ...
            case 'v':
                ...
                if ((val >= 3.8 && val < 4.0) || (val > 4.5 && val <= 4.7)) return 'status-warning';
    ...
    const renderPotCard = (pot) => {
        ...
        return (
            <div
                key={pot.id}
                onClick={() => navigate(`/pot/${pot.id}`, { state: { from: '/potline-map' } })}
                style={{
                    display: 'flex',
                    cursor: 'pointer',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                    ...
                    <div style={{
                        backgroundColor: isOffline ? offlineBg : getBackgroundColor('ce', pot.ce),
                        display: 'flex',
                        alignItems: 'center',
    ...
    const Legend = ({ style = {}, vertical = false }) => {
        ...
        const Label = ({ color, text, subText, width = '35px' }) => (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1px' }}>
                ...
                {subText && <span style={{ fontSize: '0.5rem', color: 'var(--text-secondary)', fontWeight: '600' }}>{subText}</span>}
            </div>
        );
        ...
        const HorizontalUnitSection = () => (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ fontSize: '0.7rem', fontWeight: '800', color: 'var(--text-secondary)' }}>UNIT:</div>
                <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid #94a3b8', width: '32px', borderRadius: '2px', overflow: 'hidden' }}>
                    <div style={{ background: '#e2e8f0', fontSize: '0.5rem', textAlign: 'center', color: '#64748b', fontWeight: 'bold', padding: '1px' }}>ID</div>
                    <div style={{ background: '#f0fdf4', fontSize: '0.5rem', textAlign: 'center', color: '#166534', borderTop: '1px solid #cbd5e1', padding: '1px' }}>CE</div>
                </div>
            </div>
        );
        ...
        const StatusSection = () => (
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                <div style={{ fontSize: '0.7rem', fontWeight: '800', color: 'var(--text-secondary)', minWidth: vertical ? 'auto' : 'auto' }}>STATUS:</div>
            ...
            <div style={{
                ...
                {vertical ? (
                    ...
                    <>
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '2px 0' }}>
                                ...
                                <Label color="#fcd34d" text="WARN" subText="85-90" width="45px" />
                    ...
                    <>
                        ...
                        <div style={{ width: '1px', height: '25px', background: 'var(--border-subtle)' }}></div>
                        <StatusSection />
                        <div style={{ width: '1px', height: '25px', background: 'var(--border-subtle)' }}></div>
                        ...
                        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                <div style={{ fontSize: '0.7rem', fontWeight: '800', color: 'var(--text-secondary)' }}>CE:</div>
    ...
    const calculateStats = (pIndex) => {
        // Filter pots
        let targetPots = [];
        if (pIndex === 0) targetPots = fullPotList; // Reduction / All
        else {
            if (pIndex === 1) targetPots = fullPotList.filter(p => (p.id >= 101 && p.id <= 185) || (p.id >= 201 && p.id <= 285));
            if (pIndex === 2) targetPots = fullPotList.filter(p => (p.id >= 301 && p.id <= 385) || (p.id >= 401 && p.id <= 485));
            if (pIndex === 3) targetPots = fullPotList.filter(p => (p.id >= 501 && p.id <= 585) || (p.id >= 601 && p.id <= 685));
        }
        ...
        const sumBT = targetPots.reduce((acc, p) => acc + (parseFloat(p.bt) || 0), 0);
        const sumCE = targetPots.reduce((acc, p) => acc + (parseFloat(p.ce) || 0), 0);
        const sumV = targetPots.reduce((acc, p) => acc + (parseFloat(p.v) || 0), 0);
        const sumNoise = targetPots.reduce((acc, p) => acc + (parseFloat(p.noise) || 0), 0);
        const sumM = targetPots.reduce((acc, p) => acc + (parseFloat(p.m) || 0), 0);
        const sumBathTap = targetPots.reduce((acc, p) => acc + (parseFloat(p.bath_tap) || 0), 0);
        const sumAEF = targetPots.reduce((acc, p) => acc + (parseFloat(p.aef) || 0), 0);
        const sumAEkWh = targetPots.reduce((acc, p) => acc + (parseFloat(p.ae_kwh) || 0), 0);
        const sumAge = targetPots.reduce((acc, p) => acc + (parseFloat(p.age) || 0), 0);
        const sumFe = targetPots.reduce((acc, p) => acc + (parseFloat(p.fe) || 0), 0);
        const sumSi = targetPots.reduce((acc, p) => acc + (parseFloat(p.si) || 0), 0);
        const sumSa = targetPots.reduce((acc, p) => acc + (parseFloat(p.sa) || 0), 0);
        ...
        if (contextMatches && summaryStats && summaryStats.trendData && Array.isArray(summaryStats.trendData)) {
            ...
            yestData = summaryStats.trendData.find(d => d.date && d.date.startsWith(yestStr));
        }
        ...
        return {
            ...
            currentKA,
            data: [
                { label: 'Current Eff (%)', today: avgCE.toFixed(2), yest: yestCE.toFixed(2) },
                { label: 'Bath Temp (°C)', today: avgBT.toFixed(1), yest: yestBT.toFixed(1) },
                { label: 'Voltage (V)', today: avgV.toFixed(2), yest: yestV.toFixed(2) },
                { label: 'Noise (mV)', today: avgNoise.toFixed(1), yest: yestNoise.toFixed(1) },
                { label: 'Metal Level (cm)', today: avgM.toFixed(1), yest: yestM.toFixed(1) },
                { label: 'Bath Level (cm)', today: avgBathTap.toFixed(1), yest: yestBathTap.toFixed(1) },
                { label: 'Anode Eff (Freq)', today: avgAEF.toFixed(2), yest: yestAEF.toFixed(2) },
                { label: 'AE Energy (kWh)', today: avgAEkWh.toFixed(2), yest: yestAEkWh.toFixed(2) },
                { label: 'Pot Age (days)', today: avgAge.toFixed(0), yest: yestAge.toFixed(0) },
    ...
    const PotlineCharts = ({ pots }) => {
        ...
        const ChartCard = ({ title, children, scrollable = false }) => (
            <div style={{
                ...
                <div style={{ fontSize: '0.7rem', fontWeight: '700', color: 'var(--text-primary)', marginBottom: '0.2rem' }}>{title}</div>
                <div style={{
                    flex: 1,
                    minHeight: 0,
        ...
        return (
            <div style={{ display: 'flex', gap: '0.5rem', height: '100%', flex: 1 }}>
                    ...
                    <div style={{ flex: 1, minHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                ...
                                <Tooltip
                                    contentStyle={{ background: isDarkMode ? '#334155' : 'white', border: 'none', borderRadius: '4px', fontSize: '0.7rem' }}
                                    itemStyle={{ padding: 0 }}
                                />
                            </PieChart>
                        </ResponsiveContainer>
                    ...
                    <div style={{
                        ...
                        {pieData.map(d => (
                            <div key={d.name} style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
                                <div style={{ width: 8, height: 8, background: d.color, borderRadius: 2 }}></div>
                ...
                <ChartCard title="Highest CE (Top 10)" scrollable={true}>
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={highestCE} margin={{ top: 15, right: 10, left: -25, bottom: 0 }}>
                            <XAxis
                                dataKey="name"
                                fontSize={6}
                                tickLine={false}
                                ...
                                interval={0}
                            />
                            <YAxis domain={['dataMin - 0.5', 'dataMax + 0.5']} fontSize={8} tick={false} axisLine={false} tickLine={false} />
                            <Tooltip contentStyle={{ fontSize: '0.7rem' }} cursor={{ fill: 'transparent' }} />
                            <Bar
                                dataKey="ce"
                                fill="#86efac"
                                radius={[2, 2, 0, 0]}
                                ...
                                label={{ position: 'top', fontSize: 7, fill: 'var(--text-secondary)', formatter: (val) => val.toFixed(1) }}
                            />
                        </BarChart>
                    </ResponsiveContainer>
                </ChartCard>
                ...
                <ChartCard title="Lowest CE (Bottom 10)" scrollable={true}>
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={lowestCE} margin={{ top: 15, right: 10, left: -25, bottom: 0 }}>
                            <XAxis
                                dataKey="name"
                                fontSize={6}
                                tickLine={false}
    ...
    const KPICard = ({ data }) => {
        ...
        return (
            <div style={{
                ...
                {/* Header */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '0.3rem' }}>
                    <div style={{ fontWeight: '800', fontSize: '1rem', color: 'var(--text-primary)' }}>{data.title}</div>
                    <div style={{ fontSize: '0.7rem', fontWeight: '700', color: 'var(--text-secondary)' }}>PIO = {data.count} pots</div>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.65rem' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <span style={{ width: '8px', height: '8px', background: '#334155', borderRadius: '1px', display: 'inline-block' }}></span>
                        <span style={{ fontWeight: '700', color: 'var(--text-secondary)' }}>{data.offline} pots</span>
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{
                        ...
                        background: '#86efac', color: '#14532d', fontWeight: '800', fontSize: '0.6rem',
                        padding: '4px 6px', borderRadius: '4px 4px 0 0'
                    }}>
                        <div>ITEM</div>
                        <div>{yestLabel}</div>
                        <div>{todayLabel}</div>
                        ...
                        {data.data.map((row, idx) => {
                            ...
                            return (
                                <div key={idx} style={{
                                    ...
                                    background: idx % 2 === 0 ? (isDarkMode ? '#0f172a' : '#f8fafc') : 'transparent'
                                }}>
                                    <div>{row.label}</div>
                                    <div style={{ color: 'var(--text-secondary)' }}>{row.yest}</div>
                                    <div>{row.today}</div>
                                    <div style={{ color: isUp ? '#22c55e' : '#ef4444', textAlign: 'center', fontWeight: 'bold' }}>
                                        {isUp ? '↑' : '↓'}
                                    </div>
                                </div>
                            )
    ...
    if (error) {
        return <div className="dashboard-error">Error loading data. Please try again later.</div>;
    }
    ...
    return (
        <div className={`dashboard-container ${isDarkMode ? 'dark-mode' : ''}`}>
            ...
            {loading ? (
                ...
                <div style={{ paddingBottom: '0rem', height: 'calc(100vh - 80px)', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{
                        ...
                        <button
                            ...
                            style={{ padding: '0.3rem 0.6rem', gap: '4px', fontSize: '0.75rem' }}
                        >
                            <ArrowLeft size={14} />
                            Back To Dashboard MAIN
                        </button>

                        <div style={{ color: 'var(--text-secondary)', fontWeight: '600', fontSize: '0.8rem' }}>
                            {isSearching ? `Found ${filteredDisplayPots.length}` : `${selectedPotline} (${totalPots} Pots)`}
                        </div>
                    </div>
                    ...
                    {isAllPotlines && (
                        <div style={{
                            ...
                            {kpiStats.map((stat, i) => <KPICard key={i} data={stat} />)}
                        </div>
                    )}
                        ...
                        <div style={{
                            ...
                            {isAllPotlines ? (
                                            ...
                                            <div key={potlineIdx} style={{
                                                ...
                                                <div style={{
                                                    ...
                                                    {sectionBlocks.map(block => {
                                                        const { pots1, pots2 } = getBlockDisplayItems(block, fullPotList);
                                                        const topBlockLabel = block.labelTop;
                                                        const bottomBlockLabel = block.labelBottom;

                                                        return (
                                                            <div key={block.id} className="pot-block" style={{
                                                                border: 'none',
                                                                borderRadius: '0px',
                                                                padding: '0px',
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', flex: 1, minHeight: 0 }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '1rem', height: '220px', flexShrink: 0 }}>
                                        {/* Left: KPI Card */}
                                        <div style={{ flex: '0 0 280px', display: 'flex', flexDirection: 'column' }}>
                                            <KPICard data={kpiStats.find(k => k.title === selectedPotline) || kpiStats[0]} />
                                        </div>
                                            ...
                                            <PotlineCharts pots={
                                                isAllPotlines
                                                    ...
                                                    : filteredDisplayPots.filter(p => {
                                                        if (selectedPotline === 'POTLINE 1') return (p.id >= 101 && p.id <= 285);
                                                        if (selectedPotline === 'POTLINE 2') return (p.id >= 301 && p.id <= 485);
                                                        if (selectedPotline === 'POTLINE 3') return (p.id >= 501 && p.id <= 685);
                                                        return true;
                                                    })
                                            } />
                                        </div>
                                        ...
                                        {blocks.map(block => {
                                            ...
                                            return (
                                                <div key={block.id} className="pot-block" style={{
                                                    ...
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', flex: 1, padding: '2px' }}>
                                                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(22,1fr)', gap: '1px', flex: 1, direction: 'rtl' }}>
                                                            {pots1.map(renderPotCard)}
                                                        </div>
                                                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(22,1fr)', gap: '1px', flex: 1, direction: 'rtl', marginTop: '1px' }}>
