...
import { ArrowLeft, Thermometer, Zap, AlertTriangle, Lightbulb, ChevronRight, Layers, Activity, Timer, Scale, Percent, Ruler } from 'lucide-react';
...
const PotDetailPage = ({ isDarkMode, toggleTheme }) => {
        ...
        const fetchPotDetails = async () => {
            ...
            try {
                ...
                const getParamStatus = (type, value, extraData = {}) => {
                    if (value === null || value === undefined) return { status: 'optimal' };
                    ...
                    switch (type) {
                        case 'bt': if (!std && (numVal < 940 || numVal > 960)) return { status: 'warning', message: '940-960' }; break;
                        case 'ae':
                            // Special AE Logic: Settings usually have duration or freq
                            // If checking duration from settings
                            const aeStd = settings?.['ae_dur'];
                            if (aeStd && extraData.duration > aeStd.max_val) return { status: 'warning', message: 'AE Event' };

                            if (!aeStd && extraData.duration >= 60) return { status: 'critical', message: 'CRITICAL >=60s' };
                            if (!aeStd && extraData.duration > 0) return { status: 'warning', message: 'AE Event Detected' };
                            break;
                        case 'feed': if (numVal < 95 || numVal > 105) return { status: 'warning', message: '95-105%' }; break;
                        case 'oa': if (numVal < 12 || numVal > 20) return { status: 'warning', message: '12-20' }; break;
                        case 'osp':
                            if (extraData.psp) {
                                const diff = Math.abs(numVal - extraData.psp);
                                if (diff > 0.5) return { status: 'warning', message: `Dev > 0.5` };
                            }
                            break;
                        case 'ov': if (numVal < 4.0 || numVal > 4.5) return { status: 'warning', message: '4.0-4.5V' }; break;
                        case 'avv': if (!std && (numVal < 4.1 || numVal > 4.4)) return { status: 'warning', message: '4.1-4.4V' }; break;
                        case 'current': if (numVal < 190 || numVal > 200) return { status: 'warning', message: '195±5' }; break;
                        case 'noise': if (!std && numVal > 50) return { status: 'warning', message: '<=50' }; break;
                        case 'alf3': if (numVal < 40 || numVal > 60) return { status: 'warning', message: '40-60' }; break;
                        case 'caf2': if (numVal < 3 || numVal > 6) return { status: 'warning', message: '3-6%' }; break;
                        case 'm': if (!std && (numVal <= 23 || numVal >= 27)) return { status: 'critical', message: '23-27' }; break;
                    }
                ...
                const latestPot = latestBatch.rows.find(p => p.pot_id === parseInt(id));
                ...
                const rangeData = await potService.getLayerRange(id, startDate.toISOString(), endDate);
                let rows = rangeData.rows || [];
                ...
                const ciLow = latestPot.yhat_lo !== undefined ? Number(latestPot.yhat_lo).toFixed(2) : null;
                const ciHigh = latestPot.yhat_hi !== undefined ? Number(latestPot.yhat_hi).toFixed(2) : null;
                ...
                const mappedSensors = [
                    { type: 'temperature', value: `${(latestPot.bt || 0).toFixed(0)}°C`, label: 'Bath Temperature (BT)', status: getParamStatus('bt', latestPot.bt).status, trend: calcTrend(latestPot.bt, prevPot?.bt, 0), range: '940-960' },
                    { type: 'level', value: `${(latestPot.m || 0).toFixed(1)} cm`, label: 'M (TINGGI METAL)', status: getParamStatus('m', latestPot.m).status, trend: calcTrend(latestPot.m, prevPot?.m, 1), range: '23-27' },
                    { type: 'noise', value: `${(latestPot.noise || 0).toFixed(0)} mV`, label: 'Noise', status: getParamStatus('noise', latestPot.noise).status, trend: calcTrend(latestPot.noise, prevPot?.noise, 0), range: '<=50' },
                    { type: 'osp', value: `${(latestPot.osp || 0).toFixed(2)} V`, label: 'OSP', status: getParamStatus('osp', latestPot.osp, { psp: latestPot.psp }).status, trend: '-', range: '~PSP' },
                    { type: 'voltage', value: `${(latestPot.avv || 0).toFixed(2)} V`, label: 'Average Voltage', status: getParamStatus('avv', latestPot.avv).status, trend: calcTrend(latestPot.avv, prevPot?.avv, 2), range: '4.1-4.4' },
                    { type: 'current', value: `${(latestPot.pl_current || 0).toFixed(0)} kA`, label: 'Line Current', status: getParamStatus('current', latestPot.pl_current).status, trend: '-', range: '195' },
                    { type: 'feed', value: `${(latestPot.feed_pct || 0).toFixed(2)}%`, label: 'Feeding Rate', status: getParamStatus('feed', latestPot.feed_pct).status, trend: '-', range: '95-105%' },
                    { type: 'oa', value: `${(latestPot.oa || 0).toFixed(0)}`, label: 'OA', status: getParamStatus('oa', latestPot.oa).status, trend: '-', range: '12-20' },
                    { type: 'time', value: `${latestPot.age_day || 0}d`, label: `Pot Age: ${getAgeLabel(latestPot.age_day)}`, status: 'optimal', trend: '', range: '-' },
                    { type: 'alf3', value: `${(latestPot.alf3 || 0).toFixed(0)} kg`, label: 'ALF3', status: getParamStatus('alf3', latestPot.alf3).status, trend: '-', range: '40-60' },
                    { type: 'caf2', value: `${(latestPot.caf2 || 0).toFixed(2)}%`, label: 'CAF2', status: getParamStatus('caf2', latestPot.caf2).status, trend: '-', range: '3-6%' },
                    { type: 'ae', value: `${(latestPot.ae_dur || 0).toFixed(0)}s`, label: 'Anode Effect', status: getParamStatus('ae', latestPot.ae_dur, { duration: latestPot.ae_dur }).status, trend: calcTrend(latestPot.ae_dur, prevPot?.ae_dur, 0), range: '<60s' },
                ];

                // 5. Prepare Charts Data
                const formatDay = (d) => new Date(d).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' });
                ...
                const chartRows = rows.filter(r => new Date(r.ts_5m) >= chartFilterDate);

                const chartsData = {
                    bt: chartRows.map(r => ({ day: formatDay(r.ts_5m), value: Number(r.bt) })),
                    avv: chartRows.map(r => ({ day: formatDay(r.ts_5m), value: Number(r.avv) })),
                    noise: chartRows.map(r => ({ day: formatDay(r.ts_5m), value: Number(r.noise) })),
                    m: chartRows.map(r => ({ day: formatDay(r.ts_5m), value: Number(r.m) })),
                    ae_dur: chartRows.map(r => ({ day: formatDay(r.ts_5m), value: Number(r.ae_dur) })),
                    feed_pct: chartRows.map(r => ({ day: formatDay(r.ts_5m), value: Number(r.feed_pct) })),
                    oa: chartRows.map(r => ({ day: formatDay(r.ts_5m), value: Number(r.oa) })),
                    osp: chartRows.map(r => ({ day: formatDay(r.ts_5m), value: Number(r.osp) })),
                    current: chartRows.map(r => ({ day: formatDay(r.ts_5m), value: Number(r.current || r.pl_current) })),
                    alf3: chartRows.map(r => ({ day: formatDay(r.ts_5m), value: Number(r.alf3) })),
                    caf2: chartRows.map(r => ({ day: formatDay(r.ts_5m), value: Number(r.caf2) })),
                };
                ...
                const check = (key, val, defaultMin, defaultMax, title, msgSuffix, decimals = 2) => {
                    ...
                    if (mn !== null && val < mn) {
                        mappedData.warnings.push({ title: title, message: `${msgSuffix || key} ${Number(val).toFixed(decimals)} < ${mn}` });
                    }
                    if (mx !== null && val > mx) {
                        mappedData.warnings.push({ title: title, message: `${msgSuffix || key} ${Number(val).toFixed(decimals)} > ${mx}` });
                    }
                };
                ...
                // 4. Noise
                check('noise', latestPot.noise, null, 50, "High Noise (Signal)", "Noise", 0);

                // 5. M
                check('m', latestPot.m, 23, 27, "Metal Level Critical", "M");

                // 6. Bath Level (bath_tap)
                if (latestPot.bath_tap) check('bath_tap', latestPot.bath_tap, 15, 25, "Bath Level Deviation", "Bath Level");

                // 7. AE Freq (aef)
                check('aef', latestPot.aef, null, 0.5, "High AE Frequency", "AE Freq", 1);

                // 8. AE Duration (ae_dur)
                const aeMax = settings?.['ae_dur']?.max_val ?? 0;
                if (latestPot.ae_dur > aeMax) mappedData.warnings.push({ title: "AE Detected", message: `Duration ${latestPot.ae_dur}s` });

                // 9. AE Energy (ae_kwh)
                check('ae_kwh', latestPot.ae_kwh, null, 2.0, "High AE Energy", "AE Energy");

                // 10. Pot Age
                check('age_day', latestPot.age_day, null, 2500, "Old Pot Age", "Age", 0);

                // 11. Fe
                check('fe', latestPot.fe, null, 0.20, "High Iron Content", "Fe");

                // 12. Si
                check('si', latestPot.si, null, 0.20, "High Silicon", "Si");

                // Extra: Ratio (Sa) - Check sa_in_target flag if available
                if (latestPot.sa_in_target !== undefined && latestPot.sa_in_target === 0) mappedData.warnings.push({ title: "Ratio Out of Target", message: `Sa ${latestPot.sa}%` });


                // Decision Tree Warnings (Check if AI predicts issues)
                // Rule 1: Predicted Drops
                if (latestPot.predicted_ce && latestPot.ce && (latestPot.predicted_ce < latestPot.ce - 1.5)) {
                    mappedData.warnings.push({ title: "Efficiency Drop Forecast (AI)", message: `Predicted -${(latestPot.ce - latestPot.predicted_ce).toFixed(1)}% drop.` });
                }
                // Rule 2: Instability Pattern (Noise + Voltage)
                if (latestPot.noise > 45 && latestPot.avv < 4.0) {
                    mappedData.warnings.push({ title: "Instability Pattern (AI)", message: "High Noise + Low Voltage instability." });
                }
                // Rule 3: Aging Risks
                if (latestPot.age_day > 1500 && latestPot.fe > 0.12) {
                    mappedData.warnings.push({ title: "End-of-Life Risks (AI)", message: "High Age + Iron content risk." });
                }
                ...
                try {
                    ...
                    if (!mappedData.recommendations || mappedData.recommendations.length === 0) {
    ...
    return (
        <div className={`dashboard-container ${isDarkMode ? 'dark-mode' : ''}`}>
            ...
            <div className="detail-navigation-bar" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <button
                ...
                <button className="nav-btn today-btn">
                    TODAY
                </button>

                {/* Real-time Clock */}
                <div className="real-time-clock" style={{
                    display: 'flex',
            ...
            <div className="detail-grid">
                ...
                <div className="efficiency-sensor-row">
                    ...
                    <div className="efficiency-column" style={{ display: 'flex', flexDirection: 'column', gap: '16px', minWidth: '300px' }}>
                        <EfficiencyGaugeCard title="Current Efficiency" data={data.currentEfficiency} />
                        <EfficiencyGaugeCard title="Predicted CE" data={data.predictedEfficiency} />
                    </div>
                    ...
                    <div className="sensor-grid-container">
                        {data.sensors.map((sensor, index) => {
                            ...
                            else if (sensor.type === 'current') Icon = Zap; // Re-using Zap for current
                            else if (sensor.type === 'resistance') Icon = Activity; // Re-using Activity for resistance
                            else if (sensor.type === 'power') Icon = Zap; // Re-using Zap for power
                            else if (sensor.type === 'feed') Icon = Layers; // Re-using Layers for feed
                            else if (sensor.type === 'cathode') Icon = Activity; // Re-using Activity for cathode
                            else if (sensor.type === 'acd') Icon = Ruler; // Re-using Ruler for acd

                            return (
                                <div key={index} className={`sensor-detail-card ${sensor.status}`}>
                                    ...
                                    <div className="sensor-content">
                                        ...
                                        <div className="sensor-range" style={{ fontSize: '0.75rem', opacity: 0.8, marginTop: '2px' }}>
                                            Range: {sensor.range}
                                        </div>
                                    </div>
                                </div>
                ...
                <div className="detail-section warning-section">
                    <div className="section-header text-warning" style={{ marginBottom: '1rem' }}>
                        <AlertTriangle size={20} /> Early Warning System
                    </div>

                    {(data.warnings && data.warnings.length > 0) ? (
                        <div className="warning-grid" style={{
                            display: 'grid',
                            gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))',
                            gap: '12px'
                        }}>
                            {data.warnings.map((warn, idx) => (
                                <div key={idx} className="warning-banner" style={{ margin: 0, height: '100%' }}>
                                    <div className="warning-icon-large"><AlertTriangle size={24} /></div>
                        ...
                        <div className="warning-banner optimal">
                            <div className="warning-icon-large"><Lightbulb size={24} /></div>
                            <div className="warning-content">
                                <div className="warning-title">System Normal</div>
                                <div className="warning-message">No critical warnings triggered (Signal or AI).</div>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="detail-section recommendation-section">
                    ...
                    <div className="recommendation-grid">
                        {data.recommendations.map((rec, idx) => (
                            <div key={idx} className={`recommendation-card priority-${rec.priority <= 2 ? '1' : rec.priority <= 4 ? '2' : 'normal'}`}>
                                    ...
                                    <div className="rec-content">
                                        ...
                                        <ul className="rec-actions">
                                            {rec.actions.map((action, i) => (
                                                <li key={i} className="rec-action-item">{action}</li>
                                            ))}
                                        </ul>
                                        <div className="rec-impact" style={{ marginTop: '0.5rem', fontSize: '0.85rem', color: '#15803d' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '2rem 0 1rem' }}>
                    <h3 style={{ fontSize: '1.25rem', fontWeight: 700 }}>Parameter Trends</h3>
                    <div className="time-toggle" style={{ position: 'relative', minWidth: '160px' }}>
                        <select
                            value={timeRange}
                ...
                <div className="detail-charts-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>

                    {/* Render Filtered Charts with Reference Lines */}
                    {['bt', 'm', 'noise', 'osp', 'feed_pct', 'oa', 'current', 'alf3'].map((key) => {
                        if (!data.charts[key]) return null;
                        const chartData = data.charts[key];
                        // Fallback config if not found
                        const config = data.chartConfigs[key] || { label: key.toUpperCase(), color: '#888', type: 'line' };

                        // Define Reference Lines (Normal Standards - Upper Limit Only)
                        let limitValue = null;
                        switch (key) {
                            case 'bt': limitValue = 960; break;
                            case 'm': limitValue = 27; break;
                            case 'noise': limitValue = 50; break;
                            case 'osp': limitValue = 0.5; break;
                            case 'feed_pct': limitValue = 105; break;
                            case 'oa': limitValue = 20; break;
                            case 'current': limitValue = 200; break;
                            case 'alf3': limitValue = 60; break;
                        }
                        const referenceLines = limitValue ? [{ y: limitValue, color: config.color }] : [];
            ...
            <div className="additional-params-section" style={{ marginTop: '2rem' }}>
                <h3 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '1.5rem', borderLeft: '4px solid var(--primary)', paddingLeft: '10px' }}>
                    Additional Operational Parameters
                </h3>

                {/* 1. Pot Status & Controls */}
                <div className="param-group" style={{ marginBottom: '2rem' }}>
                    <h4 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '1rem', color: 'var(--text-secondary)' }}>Pot Status & Controls</h4>
                                ...
                                <div key={idx} className="sensor-detail-card optimal compact">
                                    ...
                                    <div className="sensor-content">
                                        <div className="sensor-value">
                                            {val !== undefined && val !== null ? val : '-'} <span style={{ fontSize: '0.8rem' }}>{item.unit}</span>
                                        </div>
                ...
                <div className="param-group" style={{ marginBottom: '2rem' }}>
                    <h4 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '1rem', color: 'var(--text-secondary)' }}>Production & Physical</h4>
                    <div className="sensor-grid-container compact-grid">
                        {[
                            ...
                            return (
                                <div key={idx} className="sensor-detail-card optimal compact">
                                    ...
                                    <div className="sensor-content">
                                        <div className="sensor-value">
                                            {val !== undefined && val !== null && typeof val === 'number' ? val.toFixed(1) : (val || '-')} <span style={{ fontSize: '0.8rem' }}>{item.unit}</span>
                                        </div>
                                        <div className="sensor-label">{item.label}</div>
                                    </div>
                                </div>
                ...
                {/* 3. Anode & Mechanical */}
                <div className="param-group" style={{ marginBottom: '2rem' }}>
                    <h4 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '1rem', color: 'var(--text-secondary)' }}>Anode & Mechanical</h4>
                    <div className="sensor-grid-container compact-grid">
                        {[
                ...
                {/* 4. Chemistry Details */}
                <div className="param-group" style={{ marginBottom: '2rem' }}>
                    <h4 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '1rem', color: 'var(--text-secondary)' }}>Detailed Chemistry</h4>
                    <div className="sensor-grid-container compact-grid">
                        {[
                ...
                <div className="param-group" style={{ marginBottom: '2rem' }}>
                    <h4 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '1rem', color: 'var(--text-secondary)' }}>Electrical & Control Extras</h4>
                    <div className="sensor-grid-container compact-grid">
                        {[
            ...
            <div className="history-section" style={{ marginTop: '3rem', paddingBottom: '3rem' }}>
                <h3 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '1.5rem', borderLeft: '4px solid var(--primary)', paddingLeft: '10px' }}>
...
const EfficiencyGaugeCard = ({ title, data }) => {
    ...
    return (
        ...
        >
            <div className="card-header" style={{ color: config.text }}>{title}</div>
            <div className="gauge-container">
                <div className="gauge-circle">
                    <div className="gauge-value" style={{ color: config.color }}>{value}%</div>
...
const ChartCard = ({ title, data, color, type = 'area', subtitle = 'Weekly Trend', referenceLines = [], limit = null }) => {
        ...
        if (active && payload && payload.length) {
            return (
                <div style={{
                    ...
                    <p style={{ margin: 0, fontWeight: 600, color: 'var(--text-secondary)' }}>{label}</p>
                    <p style={{ margin: '4px 0 0', color: color, fontWeight: 700 }}>
                        Value: {payload[0].value}
                    </p>
                    {limit !== null && (
                        <p style={{ margin: '4px 0 0', color: 'var(--text-secondary)', borderTop: '1px solid var(--border-subtle)', paddingTop: '4px' }}>
