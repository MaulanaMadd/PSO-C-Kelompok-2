...
from app.ml.preprocessing import preprocess_for_prediction, PipelineConfig
    ...
    try:
        with engine.begin() as conn:
            ...
            """), {
                ...
                "avg_pred": float(np.mean(predictions)) if len(predictions) > 0 else None,
                "min_pred": float(np.min(predictions)) if len(predictions) > 0 else None,
                "max_pred": float(np.max(predictions)) if len(predictions) > 0 else None,
                "std_pred": float(np.std(predictions)) if len(predictions) > 0 else None,
                "status": status,
                "warnings": warnings,
...
def prep_like_train_for_lgbm(df_feat: pd.DataFrame, bundle: Dict) -> pd.DataFrame:
    ...
    X = df_feat.drop(columns=[c for c in drop_cols if c in df_feat.columns], errors="ignore").copy()
    ...
    for c in CAT_COLS:
        X[c] = X[c].astype("object").fillna(MISSING_TOKEN).astype(str).astype("category")
...
def run_daily_predictions():
    ...
    try:
        # 1. Load model bundle
        bundle_path = os.path.join(os.path.dirname(__file__), "app", "ml", "artifacts", "lgbm_ce_next_bundle.pkl")
        ...
        if df_raw.empty:
            ...
            log_prediction_run(
                engine, run_id, model_name, model_version, 0,
                time.time() - start_time, np.array([]),
                status="error", error_message="No data in raw.pot_daily"
            )
            return
        ...
        df_latest = df_active.sort_values(["potnum", "tgl"]).groupby("potnum").tail(1).reset_index(drop=True)
        
        print(f"Predicting for {len(df_latest)} active pots (including those with 0 CE/no tap)")
        ...
        output_df = df_latest[["potnum", "tgl", "ce_next_pred", "yhat_lo", "yhat_hi"]].copy()
        ...
        output_df = output_df.rename(columns={
            "potnum": "pot_id",
            "ce_next_pred": "yhat_ce"
        })
        output_df = output_df.drop(columns=["tgl"])
        
        # Convert pot_id back to integer
        output_df["pot_id"] = pd.to_numeric(output_df["pot_id"], errors="coerce").astype("Int64")
        ...
        potline_map["potnum"] = pd.to_numeric(potline_map["potnum"], errors="coerce").astype("Int64")
        ...
        with engine.begin() as conn:
            ...
            output_df[["pred_date", "pot_id", "potline_id", "as_of_date", "model_name", 
                       "model_version", "feature_version", "yhat_ce", "yhat_lo", "yhat_hi", "created_at"]].to_sql(
                "temp_preds",
                conn,
                if_exists="append",
                index=False,
        ...
        log_prediction_run(
            engine, run_id, model_name, model_version,
            len(output_df), execution_time, predictions,
            status=status, warnings=warnings if warnings else None
        )
        ...
        # Log error
        log_prediction_run(
            engine, run_id, "lgbm_production", "v1.0",
            0, execution_time, np.array([]),
...
def run_simple_dummy_predictions(engine):
    ...
    with engine.begin() as conn:
        ...
        df[["pred_date", "pot_id", "potline_id", "as_of_date", "model_name", 
            "model_version", "feature_version", "yhat_ce", "yhat_lo", "yhat_hi", "created_at"]].to_sql(
