...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    pool = await db.get_pool()
    async with pool.acquire() as conn:
        user = await conn.fetchrow(query, form_data.username) # username field contains email
...
async def update_user_me(user_update: schemas.UserUpdate, current_user: dict = Depends(auth.get_current_user)):
    email = current_user["email"]
