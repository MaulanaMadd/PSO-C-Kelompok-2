...
def drop_initial_cols(df: pd.DataFrame) -> pd.DataFrame:
    ...
    drop_extra = [
        "fe_charge", "n_bulat", "rod_rj", "mix_welding", "ba_clad", "jf",
        "kerak_kg", "kerak_freq", "tebl", "beto", "tacb"
    ]
    
    dup_val_col = ["fe", "si", "sa", "caf2", "sa_in_target", "s1a", "s1b"]
    
    df = df.drop(columns=[c for c in (drop_extra + dup_val_col) if c in df.columns], errors="ignore")
    return df
...
def cast_specific_cats_to_string(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.copy()
    cat_cols = [
        "potnum", "class", "pot_design", "tshift", "mt_shift",
        "ctype", "pot_status", "transition", "potday"
    ]
    for c in cat_cols:
        if c in df.columns:
...
def apply_window_features(df: pd.DataFrame, cfg: PipelineConfig) -> pd.DataFrame:
    ...
    exclude_cols = [
        "ce_event", "ce_valid_flag",
        cfg.id_col, "episode", "tgl", "gen",
        "pot_active", "startup_flag", "shutdown_flag", "idle_flag",
        "age_day", "age_month", "mt_day"
    ]
    ...
    for col in slope_cols:
        df[f"{col}_trend{cfg.slope_window}"] = (
            df.groupby(keys)[col].shift(1).rolling(cfg.slope_window).apply(rolling_slope, raw=False)
        )
...
# =========================
def preprocess_for_prediction(df: pd.DataFrame, cfg: PipelineConfig = None) -> pd.DataFrame:
