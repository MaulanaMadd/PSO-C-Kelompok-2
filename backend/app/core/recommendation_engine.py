...
class RecommendationEngine:
    ...
    def check_status(row: Dict[str, Any], limits: Dict[str, float]) -> Dict[str, bool]:
        ...
        status["FREEZE_IND"] = (
            (frozen_bath > 0) or (bath_powder > 0) or (return_crust > 0)
        )
        ...
        if oa is not None:
            ...
            status["OA_LOW"] = False
            status["OA_OK"] = True  # Assume OK if missing
        ...
        if feed_pct is not None:
            ...
            status["FEED_LOW"] = False
            status["FEED_HIGH"] = False
            status["FEED_OK"] = True
    ...
    def generate_recommendations(
        row: Dict[str, Any], history_rows: List[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if status["M_LOW"] or status["M_HIGH"]:
            actions = [
                "Koreksi Tapping/Metal sesuai target",
                "Pertahankan feeding normal",
            ]
            if status["M_LOW"]:
                actions.append("Cek indikasi kebocoran/metal run out")
            diagnosis = "Tinggi Metal (M) Abnormal"
            if status["M_LOW"]:
                diagnosis += " (Terlalu Rendah)"
            if status["M_HIGH"]:
                diagnosis += " (Terlalu Tinggi)"
        ...
        if status["AE_HIGH"] and status["OA_LOW"]:
            ...
            add_rec(
                "R1",
                "AE tinggi + OA rendah (kekurangan Al\u2082O\u2083 terlarut)",
                actions,
        ...
        # R2: AE High + OA Ok + BT Low
            ...
            actions = [
                "OSP dinaikkan bertahap untuk naikkan BT",
                "Jaga feeding tetap normal",
            ]
            if status["SA_OUT"] or status["ALF3_ZERO_RUN"]:
                actions.append("Cek Rule R13/R14 (Chemistry)")
            add_rec(
                "R2",
                "AE tinggi + OA normal + BT rendah (masalah pelarutan)",
                actions,
        ...
        # R3: AE High + Feed Low
            ...
            add_rec(
                "R3",
                "AE tinggi + Feed Rate rendah",
                [
                    "Feeding dinaikkan sampai minimal \u2265 95%",
                    "Monitor OA naik & AE turun",
                ],
                "Feed Rate \u2265 95%",
                "2-6 Jam",
                1,
        ...
        # R4: AE High + Noise High
            ...
            add_rec(
                "R4",
                "AE tinggi + Noise tinggi (Unstable)",
                [
                    "Stabilkan operasi dulu, hold perubahan besar",
                    "Cek tapping/operasi manual",
                    "Setelah noise turun, lanjut rule AE",
                ],
        ...
        # R5: BT Low
        if status["BT_LOW"] and not (
            status["AE_HIGH"] and status["OA_LOW"]
        ):  # Avoid double rec logic if purely driven by R1
            ...
            add_rec(
                "R5",
                "Bath Temperature Rendah (Dingin)",
                actions,
                f"BT {limits['BT_MIN']}-{limits['BT_MAX']}",
                "Shift / 2-6 Jam",
                2,
        ...
        # R8: AVV High
        if (
            status["AVV_HIGH"] and not status["AE_HIGH"]
        ):  # AE causes high volts naturally
            actions = ["Monitor AVV"]
            if status["BT_HIGH"]:
                actions.append("OSP Turun")
            if status["BT_LOW"]:
                actions.append("Cek Noise/Current sebelum ubah OSP")
            add_rec(
                "R8",
                "AVV Melewati Batas",
                actions,
        ...
        if not status["CURRENT_OK"]:
            add_rec(
                "R9",
                "Line Current Deviation",
                [
                    "Label 'Electrical Mode Change/Upset'",
                    "Tahan koreksi chemistry besar",
                    "Fokus stabilkan BT & Feeeding",
                ],
                "Current Stability",
                "2-6 Jam",
                4,
        ...
        if not status["CAF2_OK"]:
            add_rec(
                "R12",
                "Komposisi Bath Menyimpang (CaF2)",
        ...
        if status["FEED_HIGH"] and status["BT_LOW"]:
            ...
            add_rec(
                "R15",
                "Overfeeding + Dingin (Resiko Sludge)",
